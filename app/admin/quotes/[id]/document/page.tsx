...
export default async function QuoteDocumentPage({ params, searchParams }: PageProps) {
  const session = await requireAdminSession(OPERATIONS_ROLES);

  const [quote, profile] = await Promise.all([
    prisma.quoteRequest.findFirst({ where: { id: params.id, deletedAt: null } }),
  ...
  const exclusions = quote.exclusions || profile?.quotationFooter || 'Any items not explicitly listed in this quotation are excluded.';
  const assumptions = quote.assumptions || 'Pricing assumes reasonable site access and agreed material specifications.';
  const terms = quote.termsDisclaimer || profile?.quotationDisclaimer || 'This quotation is subject to final site verification, signed acceptance, and terms of engagement.';
    ...
    <main className="min-h-screen bg-slate-950 px-4 py-10 print:bg-white print:px-0 print:py-0">
      <div className="mx-auto max-w-5xl rounded-[2rem] border border-slate-800/70 bg-slate-950/80 p-8 text-slate-100 shadow-glow print:rounded-none print:border-0 print:bg-white print:p-8 print:text-slate-900 print:shadow-none">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3 print:hidden">
          <Link href={`/admin/quotes/${quote.id}`} className="rounded-full border border-slate-700 px-4 py-2 text-sm font-semibold text-slate-200 transition hover:border-brand-cyan/60 hover:text-white">
            Back to quote
          </Link>
          <Link href={`/admin/quotes/${quote.id}/document?print=1`} className="rounded-full bg-brand-blue px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-sky">
            Print / Save PDF
          </Link>
        </div>
          ...
          <div className="flex items-start justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400 print:text-slate-600">Professional Construction Quotation</p>
              </div>
            </div>
        ...
        <section className="mt-8 grid gap-6 sm:grid-cols-2">
          <div className="rounded-2xl border border-slate-800/70 bg-slate-900/70 p-5 print:border-slate-300 print:bg-white">
            ...
            <div className="mt-3 space-y-1 text-sm">
              <p><span className="font-semibold">Reference:</span> {quote.referenceCode}</p>
              <p><span className="font-semibold">Status:</span> {quote.status}</p>
              <p><span className="font-semibold">Issued:</span> {quote.quoteSentAt ? new Date(quote.quoteSentAt).toLocaleDateString() : new Date().toLocaleDateString()}</p>
              <p><span className="font-semibold">Valid until:</span> {validUntil.toLocaleDateString()}</p>
            </div>
          </div>
          <div className="rounded-2xl border border-slate-800/70 bg-slate-900/70 p-5 print:border-slate-300 print:bg-white">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-cyan print:text-slate-700">Client details</p>
            <div className="mt-3 space-y-1 text-sm">
              <p><span className="font-semibold">Name:</span> {quote.fullName}</p>
              <p><span className="font-semibold">Email:</span> {quote.email}</p>
              <p><span className="font-semibold">Phone:</span> {quote.phone}</p>
              <p><span className="font-semibold">Location:</span> {quote.location || 'Not specified'}</p>
            </div>
          </div>
        </section>

        <section className="mt-8 rounded-2xl border border-slate-800/70 bg-slate-900/70 p-5 print:border-slate-300 print:bg-white">
          <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-cyan print:text-slate-700">Project / service summary</p>
          <p className="mt-3 text-sm"><span className="font-semibold">Service type:</span> {quote.serviceType}</p>
          <p className="mt-1 text-sm"><span className="font-semibold">Project type:</span> {quote.projectType || 'Not specified'}</p>
          <p className="mt-1 text-sm"><span className="font-semibold">Budget guidance:</span> {quote.estimatedBudgetRange || 'Not specified'}</p>
          <p className="mt-4 whitespace-pre-line text-sm text-slate-200 print:text-slate-800">{quote.quoteSummary || quote.projectDescription}</p>
          {quote.scopeNotes ? (
        ...
        <footer className="mt-8 rounded-2xl border border-slate-800/70 bg-slate-900/70 p-5 print:border-slate-300 print:bg-white">
          ...
          <p className="mt-4 text-xs text-slate-400 print:text-slate-600">
            This quotation is generated for planning and client communication. Final contract values are confirmed after site verification and signed acceptance.
          </p>
        </footer>
      </div>
