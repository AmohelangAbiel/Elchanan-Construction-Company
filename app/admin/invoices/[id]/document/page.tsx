...
export default async function AdminInvoiceDocumentPage({ params, searchParams }: PageProps) {
    ...
    <main className="min-h-screen bg-slate-950 px-4 py-10 print:bg-white print:px-0 print:py-0">
      <div className="mx-auto max-w-5xl rounded-[2rem] border border-slate-800/70 bg-slate-950/80 p-8 text-slate-100 shadow-glow print:rounded-none print:border-0 print:bg-white print:p-8 print:text-slate-900 print:shadow-none">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3 print:hidden">
          <Link href={`/admin/invoices/${invoice.id}`} className="rounded-full border border-slate-700 px-4 py-2 text-sm font-semibold text-slate-200 transition hover:border-brand-cyan/60 hover:text-white">
            Back to invoice
          </Link>
          <Link href={`/admin/invoices/${invoice.id}/document?print=1`} className="rounded-full bg-brand-blue px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-sky">
            Print / Save PDF
          </Link>
        </div>
          ...
          <div className="flex items-start justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400 print:text-slate-600">Admin Invoice Document</p>
              </div>
            </div>
        ...
        <section className="mt-8 rounded-2xl border border-slate-800/70 bg-slate-900/70 p-5 print:border-slate-300 print:bg-white">
          ...
          <div className="mt-3 grid gap-2 text-sm text-slate-200 print:text-slate-800 sm:grid-cols-2">
            ...
            <p>Project: {invoice.deliveryProject ? invoice.deliveryProject.projectCode || invoice.deliveryProject.title : 'Not linked'}</p>
            <p>Milestone: {invoice.projectMilestone ? invoice.projectMilestone.title : 'Not linked'}</p>
        ...
        <footer className="mt-8 rounded-2xl border border-slate-800/70 bg-slate-900/70 p-5 print:border-slate-300 print:bg-white">
          ...
          <p className="mt-3 text-xs text-slate-400 print:text-slate-600">
            This document is generated for billing reference and should be used together with the admin invoice record for the latest status.
          </p>
        </footer>
      </div>
