...
export default async function AdminQuoteReportsPage({
  searchParams,
}: {
  searchParams?: Record<string, SearchParamValue>;
}) {
  const session = await requireAdminSession(REPORTING_ROLES);
  const filters = parseQuoteReportFilters(searchParams);
  const data = await getQuoteReportData(filters, { limit: 200 });

  const filterParams = new URLSearchParams();
  if (filters.status) filterParams.set('status', filters.status);
  if (filters.serviceType) filterParams.set('serviceType', filters.serviceType);
  if (filters.budgetRange) filterParams.set('budgetRange', filters.budgetRange);
  ...
  const exportHref = `/api/admin/reports/quotes/export${buildFilterQueryString(filterParams)}`;
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <section className="mb-8 rounded-[2rem] border border-white/10 bg-slate-950/85 p-8 shadow-glow">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.3em] text-brand-cyan">Quote reporting</p>
              <h1 className="mt-4 text-3xl font-semibold text-white">Estimator pipeline and conversion analytics</h1>
              <p className="mt-3 max-w-3xl text-slate-400">
                Track quote demand, budget bands, win/loss outcomes, and source quality with filterable exports.
              </p>
            </div>
        ...
        <section className="mb-6 rounded-2xl border border-slate-800/70 bg-slate-950/75 p-6 shadow-glow">
          <form method="get" className="grid gap-4 md:grid-cols-2 xl:grid-cols-7">
            ...
            <label className="block">
              ...
              <select
                name="budgetRange"
                defaultValue={filters.budgetRange || ''}
                className="mt-2 w-full rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100"
              >
            ...
            <div className="md:col-span-2 xl:col-span-7 flex flex-wrap gap-2">
              ...
              <Link href="/admin/reports/quotes" className="btn-ghost px-5 py-2.5 text-xs uppercase tracking-[0.16em]">
                Reset
              </Link>
            </div>
          </form>
        ...
        <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
          <div className="interactive-card rounded-2xl p-5">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Total quotes</p>
          ...
          <div className="interactive-card rounded-2xl p-5">
            ...
            <p className="mt-3 text-4xl font-semibold text-white">{data.summary.responded}</p>
          </div>
          <div className="interactive-card rounded-2xl p-5">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Won</p>
            <p className="mt-3 text-4xl font-semibold text-white">{data.summary.won}</p>
          </div>
          <div className="interactive-card rounded-2xl p-5">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Lost</p>
            <p className="mt-3 text-4xl font-semibold text-white">{data.summary.lost}</p>
          </div>
          <div className="interactive-card rounded-2xl p-5">
            <p className="text-xs uppercase tracking-[0.2em] text-slate-400">Win rate</p>
            <p className="mt-3 text-4xl font-semibold text-white">{data.summary.winRate}%</p>
          </div>
        </section>

        {data.summary.total === 0 ? (
          <section className="mt-6 rounded-2xl border border-slate-800/80 bg-slate-950/75 p-4 text-sm text-slate-300">
            No quote requests were found for the selected filters. Adjust the filters to restore pipeline visibility.
          </section>
        ) : null}

        <section className="mt-6 grid gap-4 lg:grid-cols-3">
          <MiniTrendChart title="Quotes over time" subtitle="Filtered range" data={data.trends} accent="sky" />
          <DistributionBars
            title="Status distribution"
            items={data.summary.statusBreakdown}
            emptyMessage="No quote statuses found for this filter set."
          />
          <DistributionBars
        ...
        <section className="mt-6 grid gap-4 lg:grid-cols-2">
          ...
          <DistributionBars
            ...
            items={data.summary.budgetDemand}
            emptyMessage="No budget records found for this filter set."
          />
        </section>
                ...
                {data.rows.length ? (
                  data.rows.map((row) => (
                    <tr key={row.id} className="border-t border-slate-800/70 text-slate-200">
                      ...
                      <td className="px-4 py-3">
                        <Link href={`/admin/quotes/${row.id}`} className="font-semibold text-white hover:text-brand-cyan">
                          {row.fullName}
                        </Link>
                      </td>
                      ...
                      <td className="px-4 py-3">{row.estimatedBudgetRange || 'Not set'}</td>
                  ...
                  <tr>
                    <td colSpan={7} className="px-4 py-8 text-center text-slate-400">
                      No quotes match the current filter selection.
                    </td>
                  </tr>
                )}
              </tbody>
