...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const requestMeta = buildRequestLogMeta(request, 'admin.contracts.update', requestId);
  ...
  const roleError = enforceAdminApiRole({
    ...
    requestId,
    requestMeta,
    unauthorizedEvent: 'admin.contract_update_unauthorized',
    forbiddenEvent: 'admin.contract_update_forbidden',
  });
  ...
  const baseReturnTo = safeRedirectPath(payload.returnTo, `/admin/contracts/${params.id}`, ['/admin/contracts']);
  ...
  const existing = await prisma.portalDocument.findFirst({
    ...
    select: {
      ...
      clientVisible: true,
      quoteRequest: {
        select: { id: true, leadId: true, convertedProject: { select: { id: true, leadId: true } } },
      },
      deliveryProject: {
  ...
  const type = (result.data.type || existing.type).toUpperCase();
  ...
  const [quote, deliveryProject, directLead] = await Promise.all([
    result.data.quoteRequestId
      ...
      : Promise.resolve(existing.quoteRequest),
    result.data.deliveryProjectId
      ...
      : Promise.resolve(existing.deliveryProject),
    result.data.leadId
  ...
  if (!resolvedLeadId) {
    return jsonError('Contract document must remain linked to a client lead.', 422, undefined, { requestId });
  }
  ...
  const approvalStatus = action === 'ARCHIVE'
    ? 'ARCHIVED'
    : action === 'RESTORE'
      ? 'DRAFT'
      : result.data.approvalStatus || existing.approvalStatus;
  ...
  await prisma.$transaction(async (tx) => {
    await tx.portalDocument.update({
      ...
      data: {
        ...
        clientResponseNote: result.data.clientResponseNote || null,
      },
    });
