...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const roleError = enforceAdminApiRole({
    ...
    unauthorizedEvent: 'admin.service_update_unauthorized',
    forbiddenEvent: 'admin.service_update_forbidden',
  });
  ...
  const returnTo = safeRedirectPath(payload.returnTo, '/admin/services', ['/admin/services']);
  ...
  if (existing.deletedAt) {
    return jsonError('Archived service must be restored before editing.', 409);
  }
  ...
  if (slugTaken) {
    return jsonError('A service with this slug already exists.', 409);
  }
