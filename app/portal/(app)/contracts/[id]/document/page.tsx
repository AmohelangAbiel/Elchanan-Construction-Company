...
export default async function PortalContractDocumentPage({ params, searchParams }: PageProps) {
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-4 py-10 print:bg-white print:px-0 print:py-0">
      <PortalApprovalTracker endpoint={`/api/portal/contracts/${document.id}/approval`} returnTo={`/portal/contracts/${document.id}/document`} />
      ...
      <div className="mx-auto max-w-5xl rounded-[2rem] border border-slate-800/70 bg-slate-950/80 p-8 text-slate-100 shadow-glow print:rounded-none print:border-0 print:bg-white print:p-8 print:text-slate-900 print:shadow-none">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3 print:hidden">
          <Link href={`/portal/contracts/${document.id}`} className="rounded-full border border-slate-700 px-4 py-2 text-sm font-semibold text-slate-200 transition hover:border-brand-cyan/60 hover:text-white">
            Back to contract
          </Link>
          <Link href={`/portal/contracts/${document.id}/document?print=1`} className="rounded-full bg-brand-blue px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-sky">
            Print / Save PDF
          </Link>
          <a href={`/api/portal/documents/${document.id}`} className="rounded-full border border-brand-cyan/45 px-4 py-2 text-sm font-semibold text-brand-cyan transition hover:border-brand-cyan/80 hover:text-white">
            Open source file
          </a>
        </div>
        ...
        <section className="mt-8 rounded-2xl border border-slate-800/70 bg-slate-900/70 p-5 print:border-slate-300 print:bg-white">
          ...
          <div className="mt-3 grid gap-2 text-sm text-slate-200 print:text-slate-800 sm:grid-cols-2">
            ...
            <p>Issued by: {document.uploadedByAdmin?.name || document.uploadedByAdmin?.email || 'Not set'}</p>
        ...
        <section className="mt-8 grid gap-6 sm:grid-cols-2">
          <div className="rounded-2xl border border-slate-800/70 bg-slate-900/70 p-5 print:border-slate-300 print:bg-white">
            <p className="text-xs font-semibold uppercase tracking-[0.2em] text-brand-cyan print:text-slate-700">Approval note</p>
            <p className="mt-3 whitespace-pre-line text-sm text-slate-200 print:text-slate-800">
              {document.clientResponseNote || 'Use the portal action panel to approve or reject this document.'}
            </p>
          </div>
          <div className="rounded-2xl border border-slate-800/70 bg-slate-900/70 p-5 print:border-slate-300 print:bg-white">
            ...
            <p className="mt-3 text-xs text-slate-400 print:text-slate-600">
              This print view supports client review and should be used together with the live approval workflow in the portal.
            </p>
          </div>
        </section>
